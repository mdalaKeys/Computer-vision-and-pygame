...
from fruitt import Fruit, fruit_names
import calculations
...
BACKGROUND_IMAGE = "images/bush-background.jpg"
    ...
    with MP_POSE.Pose(
        ...
        while cap.isOpened() and running:
            ...
            if type(start_fruit) is Fruit:
                ...
                lost_text = TITLE_FONT.render('Game Over!' if exploding else 'You Lost!', False, NINJA_RED)
...
def display_menu(fps, round_number=None, total_points=None, fruits_left=None):
    ...
    if type(total_points) is int:
        points_text =SMALL_NUMBER_FONT.render(f'Points: {total_points}', False, WHITE)
        screen.blit(points_text, (ratio(10), ratio(10)))

    if type(round_number) is int:
        round_text = MAIN_NUMBER_FONT.render(f'Round {round_number}', False, NINJA_RED)
        screen.blit(round_text, (GAME_WIDTH - round_text.get_width() - ratio(20), ratio(20)))
    else:
        starting_title = TITLE_FONT.render(f'Cut to begin!', False, NINJA_RED)
        screen.blit(starting_title, (
            round(GAME_WIDTH / 2) - starting_title.get_width() / 2,
            GAME_HEIGHT - starting_title.get_height() - ratio(30)))

    title_text = TITLE_FONT.render(f'Hands destroying fruits ', False, NINJA_RED)
